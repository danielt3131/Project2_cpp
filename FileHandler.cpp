...
void loadVolumeData(std::string &filename, std::vector<RoadVolume *> &volumeList) {
    ...
    while (std::getline(fileRead, line)) {
        ...
        int day;
        int year;
        sscanf(delim[0].c_str(), "%d/%d/%d", &month, &day, &year);
        year += 2000;
...
void loadSpeedData(std::string &filename, std::vector<RoadSpeed *> &speedList) {
    std::ifstream fileRead;
    fileRead.open(filename);
    std::string line;
    std::getline(fileRead, line);
    while (std::getline(fileRead, line)) {
        ...
        int day;
        int year;
        sscanf(delim[0].c_str(), "%d-%d-%d", &year, &month, &day);
        char *pDate = new char[10];
        sprintf(pDate, "%d/%d/%d", month, day, year);
        std::string date(pDate);
...
void writeRoadSectionData(std::vector<RoadSection *> sectionList) {
